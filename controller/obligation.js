...
export const handleUpdateObligation = catchAsync(async (req, res, next) => {
  ...
  const { amount, status, id: obligationId, transactionId, dates } = req.body;
  ...
  await Promise.all(dates.map(el => {
    return Fee.create({student:obligation.student,batch:obligation.batch,transaction_id:transactionId,amountPaid:amount / dates.length,date:new Date(el)});
  })) 
