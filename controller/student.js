...
export const handleUpdateStudent = catchAsync(async (req, res, next) => {
  ...
  await User.findByIdAndUpdate(studentId,req.body);

  await Obligation.findOneAndUpdate({student:studentId},req.body);
  res.status(200).json({ok:true});
});

export const handleUpdateStudentsBulk = catchAsync(async (req, res, next) => {
  const { id, role } = req.user;
  const {students,batch,allocatedHub} = req.body;
  
  if (role !== 'admin') return res.status(401).json({ ok: false });

  await User.updateMany({_id:{$in:students}},{batch,allocatedHub});

  await Obligation.updateMany({student:{$in:students}},{batch,allocatedHub});
  res.status(200).json({ok:true});
});
...
export const handleChangeMultipleDiary = catchAsync(async (req, res, next) => {
  ...
  await User.updateMany(
    { role: "student", _id: { $in: studentsId } },
    { teacher: teacherId },
  );
  res.status(200).json({ ok: true });
});
export const handleAssignMultipleProxies = catchAsync(
  async (req, res, next) => {
    const { teacherId, studentsId } = req.body;
    const { id, role } = req.user;
...
export const handleAssignProxy = catchAsync(async (req, res, next) => {
  ...
  if (user.role === "admin")
    return res
      .status(400)
      .json({ ok: false, message: "can't assign proxy to admins" });
...
export const handleGetStudents = catchAsync(async (req, res, next) => {
  ...
  if (role === "teacher") {
    ...
    filter.$or = [{ teacher: id }, { proxyTeacher: id }];
    filter.role = "student";
...
export const handleUpdateStudentsBatch = catchAsync(async (req, res, next) => {
  const { id, role } = req.user;
  const {obligations,batch} = req.body;
  const obligationsArr = obligations.map(el => el.id);
  let hubAllocated;

  
  const students = obligations.map(el => el.studentId);
  await Obligation.updateMany({_id:{$in:obligationsArr}},{$set:{batch}});
  await User.updateMany({_id:{$in:students}},{$set:{batch}});
  
  res.status(200).json({ ok: true });
});

// export const handleUpdateStudentBatch = catchAsync(async (req, res, next) => {
//   const { id, role } = req.user;
//   const {obligations,batch} = req.body;
//   const obligationsArr = obligations.map(el => el.id);
//   let hubAllocated;
